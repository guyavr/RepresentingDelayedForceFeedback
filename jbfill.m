...
if nargin<5;edge=[1  1  1];end  %dfault edge color is black
if nargin<4;color=[0.8 0.8 0.8];end %default color is blue

if(find(isnan(upper))) % if there are Nan in the Vector
    upper=upper(1: find(isnan(upper) , 1 , 'first')-1);
    lower=lower(1: find(isnan(lower) , 1 , 'first')-1);
    xpoints=xpoints(1: length(lower));
end
